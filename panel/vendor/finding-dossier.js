...
(function (global) {
  ...
  var CATALOG = [
    ...
    {
      re: /^AD:\s*\d+ cuenta\(s\) AS-REP roastable/i,
      cwe: ["CWE-308"],
      owasp: "A07:2021 Identification and Authentication Failures",
      mitre: [{ id: "T1558.004", name: "Steal or Forge Kerberos Tickets: AS-REP Roasting", tactic: "Credential Access" }],
      govKey: "misconfig",
      gdpr: ["Art. 32"],
      iso: ["A.8.5", "A.8.9"],
      ens: "Alto",
      ...
      impact: { confidentiality: "high", integrity: "none", availability: "none" },
      narrative: {
        es: "Cuentas con UF_DONT_REQUIRE_PREAUTH permiten pedir AS-REP sin conocer la contraseña (GetNPUsers -no-pass). Dark Spear no lanza hashcat: solo marca la superficie roastable.",
        en: "Accounts with UF_DONT_REQUIRE_PREAUTH allow AS-REP without knowing the password (GetNPUsers -no-pass). Dark Spear does not run hashcat: it only flags the roastable surface.",
      },
      exec: {
        es: "AS-REP roastable. Quitar DONT_REQ_PREAUTH y rotar contraseñas de esas cuentas.",
        en: "AS-REP roastable. Clear DONT_REQ_PREAUTH and rotate those account passwords.",
      },
      steps: {
        es: ["Set-ADAccountControl -DoesNotRequirePreAuth $false.", "Forzar cambio de password.", "Re-ejecutar GetNPUsers -no-pass hasta que no haya $krb5asrep$."],
        en: ["Set-ADAccountControl -DoesNotRequirePreAuth $false.", "Force a password change.", "Re-run GetNPUsers -no-pass until no $krb5asrep$."],
      },
    },
    {
      re: /^AD:\s*\d+ SPN\(s\) Kerberoastable|^AD:\s*cuentas con SPN detectadas/i,
      cwe: ["CWE-522"],
      owasp: "A07:2021 Identification and Authentication Failures",
      mitre: [{ id: "T1558.003", name: "Steal or Forge Kerberos Tickets: Kerberoasting", tactic: "Credential Access" }],
      govKey: "misconfig",
      gdpr: ["Art. 32"],
      iso: ["A.8.5"],
      ens: "Medio",
      ...
      impact: { confidentiality: "high", integrity: "none", availability: "none" },
      narrative: {
        es: "GetUserSPNs listó SPNs sin pedir TGS (-request omitido): inventario Kerberoastable. Contraseñas de servicio cortas son el riesgo real.",
        en: "GetUserSPNs listed SPNs without requesting a TGS (-request omitted): Kerberoastable inventory. Short service passwords are the real risk.",
      },
      exec: {
        es: "SPNs enumerados. Usar gMSA o passwords ≥25 y auditar SPNs huérfanos.",
        en: "SPNs enumerated. Prefer gMSA or ≥25-char passwords and audit orphan SPNs.",
      },
      steps: {
        es: ["Inventariar cuentas con servicePrincipalName.", "Migrar a gMSA donde sea posible.", "No solicitar TGS en masa sin aprobación."],
        en: ["Inventory accounts with servicePrincipalName.", "Migrate to gMSA where possible.", "Do not mass-request TGS without approval."],
      },
    },
    {
      re: /^AD:\s*ADCS template/i,
      cwe: ["CWE-284"],
      owasp: "A01:2021 Broken Access Control",
      mitre: [{ id: "T1649", name: "Steal or Forge Authentication Certificates", tactic: "Credential Access" }],
      govKey: "misconfig",
      gdpr: ["Art. 32"],
      iso: ["A.8.9", "A.5.18"],
      ens: "Alto",
      ...
      impact: { confidentiality: "high", integrity: "high", availability: "none" },
      narrative: {
        es: "certipy find -vulnerable detectó plantillas ESC. Solo enumeración (-stdout); no se solicitó certificado abusivo ni se escaló a DA.",
        en: "certipy find -vulnerable detected ESC templates. Enumeration only (-stdout); no abusive certificate was requested and no DA path was executed.",
      },
      exec: {
        es: "ADCS vulnerable. Auditar enrollment y remediaciones ESC1–ESC8 (SpecterOps).",
        en: "Vulnerable ADCS. Audit enrollment and ESC1–ESC8 remediations (SpecterOps).",
      },
      steps: {
        es: ["Revisar plantillas con enrollee supplies subject.", "Restringir Enrollment Agents.", "Re-ejecutar certipy find -vulnerable tras el cambio."],
        en: ["Review templates with enrollee supplies subject.", "Restrict Enrollment Agents.", "Re-run certipy find -vulnerable after changes."],
      },
    },
    {
    ...
    {
      re: /^AD:\s*GPP cpassword|^AD:\s*GPP autologon/i,
      cwe: ["CWE-260", "CWE-522"],
      owasp: "A07:2021 Identification and Authentication Failures",
      mitre: [{ id: "T1552.006", name: "Unsecured Credentials: Group Policy Preferences", tactic: "Credential Access" }],
      govKey: "misconfig",
      gdpr: ["Art. 32"],
      iso: ["A.8.3", "A.8.5"],
      ens: "Alto",
      ...
      impact: { confidentiality: "high", integrity: "high", availability: "none" },
      narrative: {
        es: "GPP en SYSVOL con cpassword o autologon: cualquier cuenta de dominio con lectura de SYSVOL puede recuperar la contraseña (AES conocido desde MS14-025). Dark Spear solo leyó SYSVOL; no usó el secreto.",
        en: "GPP in SYSVOL with cpassword or autologon: any domain account with SYSVOL read can recover the password (AES known since MS14-025). Dark Spear only read SYSVOL; it did not use the secret.",
      },
      exec: {
        es: "Secreto GPP en SYSVOL. Borrar el XML, rotar la cuenta y no volver a guardar passwords en GPP.",
        en: "GPP secret in SYSVOL. Delete the XML, rotate the account, and never store passwords in GPP again.",
      },
      steps: {
        es: ["Eliminar Groups/Services/Registry.xml con secretos de SYSVOL.", "Rotar todas las cuentas afectadas.", "Preferir LAPS/gMSA."],
        en: ["Remove Groups/Services/Registry.xml secrets from SYSVOL.", "Rotate all affected accounts.", "Prefer LAPS/gMSA."],
      },
    },
    {
      re: /^AD:\s*\d+ trust\(s\)|^AD:\s*trusts de dominio/i,
      cwe: ["CWE-284"],
      owasp: "A01:2021 Broken Access Control",
      mitre: [{ id: "T1482", name: "Domain Trust Discovery", tactic: "Discovery" }],
      govKey: "context",
      gdpr: ["Art. 32"],
      iso: ["A.5.9", "A.8.20"],
      ens: "Medio",
      ...
      impact: { confidentiality: "low", integrity: "none", availability: "none" },
      narrative: {
        es: "Trusts LDAP inventariados (trustedDomain). Útiles para paths cross-forest; Dark Spear no ejecutó raiseChild ni SID history abuse.",
        en: "LDAP trusts inventoried (trustedDomain). Useful for cross-forest paths; Dark Spear did not run raiseChild or SID history abuse.",
      },
      exec: {
        es: "Trusts visibles. Auditar dirección/transitividad y retirar trusts obsoletos.",
        en: "Trusts visible. Audit direction/transitivity and remove stale trusts.",
      },
      steps: {
        es: ["Documentar cada trust con el cliente.", "SID filtering en trusts externos.", "Eliminar trusts no usados."],
        en: ["Document each trust with the client.", "SID filtering on external trusts.", "Remove unused trusts."],
      },
    },
    {
      re: /^AD:\s*.*PASSWD_NOTREQD|^AD:\s*cuentas con password not required/i,
      cwe: ["CWE-521"],
      owasp: "A07:2021 Identification and Authentication Failures",
      mitre: [{ id: "T1078.002", name: "Valid Accounts: Domain Accounts", tactic: "Persistence" }],
      govKey: "misconfig",
      gdpr: ["Art. 32"],
      iso: ["A.8.5"],
      ens: "Alto",
      ...
      impact: { confidentiality: "high", integrity: "high", availability: "none" },
      narrative: {
        es: "UF_PASSWD_NOTREQD permite autenticar sin contraseña. Dark Spear solo enumeró el flag; no inició sesión con esas cuentas.",
        en: "UF_PASSWD_NOTREQD allows authentication without a password. Dark Spear only enumerated the flag; it did not log on as those accounts.",
      },
      exec: {
        es: "Cuentas sin password requerido. Quitar el flag y forzar password.",
        en: "Accounts without required password. Clear the flag and force a password.",
      },
      steps: {
        es: ["Set-ADAccountControl -PasswordNotRequired $false.", "Reset-ADAccountPassword.", "Re-enumerar --password-not-required."],
        en: ["Set-ADAccountControl -PasswordNotRequired $false.", "Reset-ADAccountPassword.", "Re-run --password-not-required."],
      },
    },
    {
      re: /^AD:\s*MachineAccountQuota/i,
      cwe: ["CWE-269"],
      owasp: "A01:2021 Broken Access Control",
      mitre: [{ id: "T1136.002", name: "Create Account: Domain Account", tactic: "Persistence" }],
      govKey: "misconfig",
      gdpr: ["Art. 32"],
      iso: ["A.8.2"],
      ens: "Medio",
      ...
      impact: { confidentiality: "low", integrity: "low", availability: "none" },
      narrative: {
        es: "ms-DS-MachineAccountQuota > 0 permite a usuarios crear computer objects (base de RBCD). Dark Spear no creó ninguna máquina.",
        en: "ms-DS-MachineAccountQuota > 0 lets users create computer objects (RBCD base). Dark Spear created none.",
      },
      exec: {
        es: "MAQ abierto. Poner MachineAccountQuota a 0 si no hay requisito.",
        en: "Open MAQ. Set MachineAccountQuota to 0 unless required.",
      },
      steps: {
        es: ["Set-ADDomain -Identity . -Replace @{ms-DS-MachineAccountQuota=0}.", "Re-verificar con netexec -M maq."],
        en: ["Set-ADDomain -Identity . -Replace @{ms-DS-MachineAccountQuota=0}.", "Re-check with netexec -M maq."],
      },
    },
    {
      re: /^AD:\s*Print Spooler activo/i,
      cwe: ["CWE-284"],
      owasp: "A01:2021 Broken Access Control",
      mitre: [{ id: "T1005", name: "Data from Local System", tactic: "Collection" }],
      govKey: "misconfig",
      gdpr: ["Art. 32"],
      iso: ["A.8.9"],
      ens: "Medio",
      ...
      impact: { confidentiality: "low", integrity: "low", availability: "none" },
      narrative: {
        es: "Spooler activo = superficie de coercion (PrinterBug). Dark Spear solo detectó el servicio; no envió coerce.",
        en: "Active spooler = coercion surface (PrinterBug). Dark Spear only detected the service; it did not coerce.",
      },
      exec: {
        es: "Spooler on. Deshabilitar en DCs; SMB signing on.",
        en: "Spooler on. Disable on DCs; SMB signing on.",
      },
      steps: {
        es: ["Stop-Service Spooler; Set-Service -StartupType Disabled en DCs.", "Re-check netexec -M spooler."],
        en: ["Stop-Service Spooler; Set-Service -StartupType Disabled on DCs.", "Re-check netexec -M spooler."],
      },
    },
    {
      re: /^AD:\s*LAPS legible/i,
      cwe: ["CWE-522", "CWE-269"],
      owasp: "A01:2021 Broken Access Control",
      mitre: [{ id: "T1078.002", name: "Valid Accounts: Domain Accounts", tactic: "Privilege Escalation" }],
      govKey: "misconfig",
      gdpr: ["Art. 32"],
      iso: ["A.5.18", "A.8.5"],
      ens: "Alto",
      ...
      impact: { confidentiality: "high", integrity: "high", availability: "none" },
      narrative: {
        es: "La cuenta de assessment lee ms-MCS-AdmPwd/msLAPS. Eso es privilegio excesivo. Dark Spear no publica el secreto en el título del hallazgo.",
        en: "The assessment account can read ms-MCS-AdmPwd/msLAPS. That is excessive privilege. Dark Spear does not put the secret in the finding title.",
      },
      exec: {
        es: "LAPS readable. Restringir ACLs y rotar passwords de hosts afectados.",
        en: "LAPS readable. Tighten ACLs and rotate passwords on affected hosts.",
      },
      steps: {
        es: ["Auditar whoCanReadLAPS.", "Quitar la cuenta de assessment de lectores LAPS.", "Rotar LAPS en hosts listados."],
        en: ["Audit whoCanReadLAPS.", "Remove the assessment account from LAPS readers.", "Rotate LAPS on listed hosts."],
      },
    },
    {
      ...
      cwe: ["CWE-287"],
      owasp: "A07:2021 Identification and Authentication Failures",
      mitre: [{ id: "T1021.006", name: "Remote Services: Windows Remote Management", tactic: "Lateral Movement" }],
      govKey: "misconfig",
      gdpr: ["Art. 32"],
      iso: ["A.8.5", "A.8.20"],
      ens: "Alto",
      ...
      impact: { confidentiality: "high", integrity: "high", availability: "none" },
      narrative: {
        es: "netexec winrm validó credenciales (Pwn3d! = admin). Dark Spear no abre evil-winrm ni ejecuta comandos: solo check de auth.",
        en: "netexec winrm validated credentials (Pwn3d! = admin). Dark Spear does not open evil-winrm or run commands: auth check only.",
      },
      exec: {
        es: "WinRM alcanzable con esas creds. Restringir 5985/5986 y rotar si no debía.",
        en: "WinRM reachable with those creds. Restrict 5985/5986 and rotate if unintended.",
      },
      steps: {
        es: ["Cerrar WinRM al perímetro no admin.", "Privileged Access Workstations.", "No usar evil-winrm en el playbook automático."],
        en: ["Close WinRM to non-admin nets.", "Privileged Access Workstations.", "Do not run evil-winrm in the automatic playbook."],
      },
    },
    {
      re: /^AD:\s*sesión nula SMB|^AD:\s*\d+ share\(s\) SMB|^AD:\s*bind LDAP|^AD:\s*\d+ usuario\(s\) enumerado|^AD:\s*\d+ usuario\(s\) vía RPC/i,
      cwe: ["CWE-200"],
      owasp: "A01:2021 Broken Access Control",
    ...
    dvwaCard({
      ...
      cwe: [],
      owasp: "N/A",
      mitre: [{ id: "T1195.001", name: "Compromise Software Supply Chain", tactic: "Initial Access" }],
      refs: [{ label: "Semgrep Registry", href: "https://semgrep.dev/explore" }],
      narrative: {
        es: "Análisis estático (semgrep) confirmó este patrón contra código fuente real, filtrado por otra vulnerabilidad de exposición (backup, source map, config reconstruible). El CWE/OWASP exacto varía por regla — viene en la descripción del propio hallazgo, generado por la regla que hizo match, no por este motor.",
        en: "Static analysis (semgrep) confirmed this pattern against real source code, leaked via another exposure vulnerability (backup, source map, reconstructible config). The exact CWE/OWASP varies per rule — it comes from the finding's own description, generated by the rule that matched, not by this engine.",
      },
      exec: {
        es: "Patrón de código inseguro confirmado por análisis estático sobre código fuente realmente filtrado. Ver la descripción del hallazgo para el CWE/OWASP específico.",
        en: "Insecure code pattern confirmed by static analysis on actually-leaked source code. See the finding description for the specific CWE/OWASP.",
      },
      steps: {
        es: ["Ver el nombre de fichero y línea en el título del hallazgo.", "Aplicar el fix sugerido si la descripción incluye uno.", "Confirmar que el fichero filtrado no queda accesible públicamente (la causa raíz es la exposición, no solo el patrón de código)."],
        en: ["Check the filename and line in the finding title.", "Apply the suggested fix if the description includes one.", "Confirm the leaked file is no longer publicly accessible (the root cause is the exposure, not just the code pattern)."],
      },
    }),
  ];
