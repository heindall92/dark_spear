...
async function main() {
  ...
  if (!token) {
    document.getElementById("app").textContent =
      "Falta el token de sesión. Abre la URL exacta que imprime bridge.py al arrancar (incluye ?token=...).";
    return;
  }
  ...
  document.getElementById("start-btn").onclick = async () => {
    ...
    if (selectedLabels.size === 0) {
      alert("Elige al menos una API key en \"Gestionar keys\" antes de arrancar.");
      return;
    }
